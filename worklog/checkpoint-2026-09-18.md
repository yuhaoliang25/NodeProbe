...
- data/reputation.json：历史上曾使用的节点信誉状态文件；已移除，现由 `data/node-pool.json` 负责节点资产与生命周期。
...
当前状态：
- 历史上的 Node Reputation：评价节点本身；该独立节点信誉体系后续已移除，当前不再作为独立状态层。
...
这层 discovery memory 不参与 Source Reputation，也不参与当前 Node Pool 生命周期判断。
